...
void new_mpc::rbt1_odom_Callbck(const nav_msgs::msg::Odometry::SharedPtr msg)
{
    buf.lock();
...
void new_mpc::rbt1_ref_traj_Callbck(const nav_msgs::msg::Path::SharedPtr msg)
{
    buf.lock();
...
void new_mpc::cmd_vel_msg_set()
{
    rbt1_cmd_msg.linear.x = (double)U0(0,0);
    rbt1_cmd_msg.angular.z = (double)U0(1,0);
}

void new_mpc::ref_traj_msg_set()
{
    rbt1_ref_msg.header.frame_id = "odom";
    rbt1_ref_msg.header.stamp = ros2time.now();
    for(int n = 0; n < N; ++n)
    {
        geometry_msgs::msg::PoseStamped n_path;
        n_path.pose.position.x = ref_path[n][0];
        n_path.pose.position.y = ref_path[n][1];
        rbt1_ref_msg.poses.emplace_back(n_path);
    }
}

void new_mpc::predictive_traj_msg_set()
{
...
void new_mpc::current_pos_msg_set()
{
...
void new_mpc::call_solver()
{
    solver_result = solver(args);
}

void new_mpc::reshape_and_init_opt_variable()
{
...
void new_mpc::setting_reference()
{
    for(int n = 0; n < N; ++n)
    {
        double roll, pitch;
...
void new_mpc::setting_solver()
{  
    // 초기 목적 함수를 0으로 설정
    casadi::SX obj = 0;
    // 제약 조건 벡터 초기화
    casadi::SXVector g_vec;
    // 상태 변수와 매개변수 간의 오차 벡터를 계산하여 제약 조건 벡터에 추가
    for(int i = 0; i < n_states; ++i)
    {
        g_vec.emplace_back(X(i,0) - P(i));
    }
    // 제약 조건 벡터를 수직으로 쌓아 하나의 벡터로 합침
    ...
    for(int n = 0; n < N; ++n)
    {
        ...
        // 상태 변수와 제어 변수의 오차 벡터 계산 및 목적 함수에 추가
        if(n >= N - 1)
        {
...
void new_mpc::rbt1_ref_traj_update()
{
...
void new_mpc::init()
{
    rbt1_odom_sub = this->create_subscription<nav_msgs::msg::Odometry>("/odom",1,std::bind(&new_mpc::rbt1_odom_Callbck,this,_1));
    rbt1_ref_traj_sub = this->create_subscription<nav_msgs::msg::Path>("/rbt1/ref_trajectory",1,std::bind(&new_mpc::rbt1_ref_traj_Callbck,this,_1));
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<new_mpc>();
    node -> init();
    node -> run();
    return 0;
}
